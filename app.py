...
KALSHI_BASE_URL: str = os.getenv(“KALSHI_BASE_URL”, “https://trading-api.kalshi.com”)  # Production API
...
BTC_ONLY_MODE: bool = os.getenv("BTC_ONLY_MODE", "false").lower() == "true"  # Default OFF to show all markets
    ...
    try:
        ...
        async with session.get(url, params=params, headers=self._get_headers()) as resp:
            if resp.status == 200:
                ...
                logger.info(f"Kalshi returned {len(markets)} markets")
                return markets
            elif resp.status == 401:
                logger.warning("Kalshi 401 - token expired, re-authenticating...")
                await self.login()
                # Retry once after re-auth
                async with session.get(url, params=params, headers=self._get_headers()) as resp2:
                    if resp2.status == 200:
                        data = await resp2.json()
                        return data.get("markets", [])
                    logger.error(f"Kalshi retry failed: {resp2.status}")
                    return []
            else:
                body = await resp.text()
                logger.error(f"Failed to fetch markets: {resp.status} - {body[:200]}")
                return []
    except Exception as e:
...
async def fetch_real_markets(self):
    ...
    try:
        ...
        if markets:
            ...
            self.log(f"Fetched {len(self.markets)} {mode_str} markets from Kalshi (skipped {skipped})", "✅")
            
            # If BTC filter removed all markets, disable filter and retry
            if len(self.markets) == 0 and config.BTC_ONLY_MODE and skipped > 0:
                self.log(f"No BTC markets found in {skipped} total markets - disabling BTC filter", "⚠️")
                config.BTC_ONLY_MODE = False
                return await self.fetch_real_markets()  # Retry without filter
...
def _process_market_check(self, market, yes, no):
    ...
    # Update price chart data for BTC/crypto markets
    if market.get('type', '') in ('CRYPTO', 'BTC') or 'btc' in market.get('question', '').lower():
        self.update_btc_price(yes)
...
def set_speed(self, ms):
    """Set polling speed"""
    self.poll_interval_ms = max(100, min(5000, ms))
